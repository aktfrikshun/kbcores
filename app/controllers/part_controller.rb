class PartController < ApplicationController
  ...
  def index
    ...
    if session[:trans_types] == nil
      @sql = "SELECT DISTINCT TRANS_TYPE FROM parts ORDER BY TRANS_TYPE "
      @trans_types = Array.new
      @results = ActiveRecord::Base.connection().execute(@sql)
      @results.each do  |row| 
        if row[0]
          @trans_types. << row[0].upcase
        end
      end
      session[:trans_types] = @trans_types
    end
    if session[:last_page] == "Admin_Pictures"
      session[:this_page] = nil
    end
    session[:last_page] = "Part"
  ...
  def refresh
    ...
    @translator = Translations.new
    redirect_to :controller => "Part", :action => :index, :rid => @rid, :refresh => "Y"
  end  
  def reset_search
